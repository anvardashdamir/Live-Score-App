...
class SegmentedHeaderView: UICollectionReusableView {
    ...
    private let segmentedControl: UISegmentedControl = {
        let control = UISegmentedControl(items: CompetitionDetailViewController.Tab.allCases.map { $0.title })
...
extension CompetitionDetailViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        header.configure { [weak self] index in
            self?.selectedTab = CompetitionDetailViewController.Tab(rawValue: index) ?? .results
